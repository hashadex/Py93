                ...
                if (success) {
                    ...
                    function compile() {
                        if (outConts == 'null') {
                            ...
                            $log.green('Finished compilation.\nCompiled file path: /a/' + compNameFin + '\nWrite "$explorer.refresh();" to the terminal to make compiled file visible.'); // FIXME: Need to fix a bug when using $explorer.refresh() in the script does nothing // FIXME: Need to fix a bug when $file.delete(*args*) in the script does nothing
                        }
                    }
